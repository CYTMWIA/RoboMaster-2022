#ifndef __NERV_LOGGING_HPP__
#define __NERV_LOGGING_HPP__
...
#define NERV_LOG(type_name, color, fmsg, ...)                                          \
  fmt::print(color, "[{:%H:%M:%S}][" type_name "][{}] " fmsg "\n", __LOG_CURRENT_TIME, \
             basename_constexpr(__FILE__), ##__VA_ARGS__)

#define NERV_ERROR(fmsg, ...) NERV_LOG("ERROR", fg(fmt::color::red), fmsg, ##__VA_ARGS__)

#define NERV_WARN(fmsg, ...) NERV_LOG("WARN", fg(fmt::color::yellow), fmsg, ##__VA_ARGS__)

#define NERV_INFO(fmsg, ...) NERV_LOG("INFO", fg(fmt::color::white), fmsg, ##__VA_ARGS__)

#define NERV_DEBUG(fmsg, ...) NERV_LOG("DEBUG", fg(fmt::color::gray), fmsg, ##__VA_ARGS__)
