...
#ifndef __RM_COMMON_FILTER_ADAPTIVE_EKF_HPP__
#define __RM_COMMON_FILTER_ADAPTIVE_EKF_HPP__
...
class AdaptiveEKF
{
 ...
 public:
  ...
  {
    for (int i = 0; i < N_X; i++) Q(i, i) = vec[i];
  }

  template <typename T>
  void set_r_diag(std::vector<T> vec)
  {
    for (int i = 0; i < N_Y; i++) R(i, i) = vec[i];
  }
