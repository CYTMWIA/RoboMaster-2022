#ifndef __RM_DETECT_CV_UTIL_HPP__
#define __RM_DETECT_CV_UTIL_HPP__
...
cv::Mat awakenlion_threshold(const cv::Mat &src, double gray_max_ = 0.6, double gray_avg_ = 0.4,
                             double ch0 = 0.1, double ch1 = 0.2, double ch2 = 0.7);
...
void calc_gray_hist(const cv::Mat &src, T *res)
{
  ...
  for (int i = 0; i < src.rows; i++)
    for (int j = 0; j < src.cols; j++) res[*(src.data + i * src.cols + j)] += 1;
}

inline double distance(cv::Point2f p1, cv::Point2f p2) { return abs(cv::norm(p1 - p2)); }
