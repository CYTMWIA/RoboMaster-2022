 ...
 */
int32_t find_mem(const uint8_t *mem, const size_t mem_size, const uint8_t *target, const size_t target_size)
{
    int i, j;
...
void CmdToCv_make(const CmdToCv *cmd, uint8_t *data)
{
    ...
    memcpy(data, kHeadingBytes, kHeadingBytesCount);    data += kHeadingBytesCount;
    memcpy(data, &(__mode), 1);                         data += 1;
    memcpy(data, &(cmd->bullet_speed), 4);              data += 4;
    memcpy(data, &(cmd->pitch), 4);                     data += 4;
    memcpy(data, &(cmd->x_acc), 4);                     data += 4;
    memcpy(data, &(cmd->y_acc), 4);                     data += 4;
...
int32_t CmdToCv_parse(CmdToCv *cmd, uint8_t *data, uint32_t data_len)
{
    ...
    memcpy(&(cmd->pitch), data, 4);         data += 4;
    memcpy(&(cmd->x_acc), data, 4);         data += 4;
