...
class DahuaCapture::Impl
{
 ...
 public:
  Impl()
  {
    ...
    if (devices.nDevNum < 1) __LOG_ERROR_AND_EXIT("未发现设备");
    ...
    if (IMV_Open(dev_) != IMV_OK) __LOG_ERROR_AND_EXIT("打开相机失败");

    if (IMV_StartGrabbing(dev_) != IMV_OK) __LOG_ERROR_AND_EXIT("拉流失败");
  }
  ...
  {
    if (IMV_SetEnumFeatureValue(dev_, "BalanceRatioSelector", 0) == IMV_OK &&
        IMV_SetDoubleFeatureValue(dev_, "BalanceRatio", red) == IMV_OK &&
        IMV_SetEnumFeatureValue(dev_, "BalanceRatioSelector", 1) == IMV_OK &&
        IMV_SetDoubleFeatureValue(dev_, "BalanceRatio", green) == IMV_OK &&
        IMV_SetEnumFeatureValue(dev_, "BalanceRatioSelector", 2) == IMV_OK &&
        IMV_SetDoubleFeatureValue(dev_, "BalanceRatio", blue) == IMV_OK)
    {
      return true;
    }
    else
  ...
  cv::Mat next()
  {
    ...
    if (IMV_GetFrame(dev_, &frame, 500) != IMV_OK) throw std::runtime_error("获取图像失败");
    ...
    if (IMV_PixelConvert(dev_, &param) != IMV_OK) __LOG_WARNING("转换像素失败");

    if (IMV_ReleaseFrame(dev_, &frame) != IMV_OK) throw std::runtime_error("释放图像缓存失败");
...
bool DahuaCapture::set_exposure_time(float time) { return pimpl->set_exposure_time(time); }

bool DahuaCapture::set_gain(float gain) { return pimpl->set_gain(gain); }
...
cv::Mat DahuaCapture::next() { return pimpl->next(); }
}  // namespace rm_autoaim