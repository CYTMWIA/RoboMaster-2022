...
namespace rmcv::detect
{
    ...
    void generate_yolox_proposals(const std::vector<GridAndStride> &grid_strides, const float *feat_ptr, float prob_threshold, std::vector<BoundingBox> &bboxes)
    {
        ...
        for (int anchor_idx = 0; anchor_idx < num_anchors; anchor_idx++)
        {
            ...
            for (int class_idx = 0; class_idx < MODEL_NUM_CLASSES; class_idx++)
            {
                ...
                if (box_prob > prob_threshold)
                {
                    ...
                    // bbox.pts[1] = cv::Point2f(x_center - w_half, y_center + h_half); // 之后会被覆盖，所以这里就不填写了
                    // bbox.pts[3] = cv::Point2f(x_center + w_half, y_center - h_half); // 同上
    ...
    void nms_sorted_bboxes(const std::vector<BoundingBox> &bboxes, std::vector<int> &picked, float nms_threshold)
    {
        picked.clear();
        ...
        for (int i = 0; i < n; i++)
        {
            areas[i] = (bboxes[i].pts[2].x - bboxes[i].pts[0].x) * (bboxes[i].pts[2].y - bboxes[i].pts[0].y);
        }
    ...
    void decode_outputs(const float *prob, std::vector<BoundingBox> &bboxes, float scale, const int img_w, const int img_h)
    {
        ...
        std::sort(proposals.begin(), proposals.end(), [](BoundingBox a, BoundingBox b)
