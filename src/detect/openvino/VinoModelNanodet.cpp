...
namespace rmcv::detect
{
    ...
    {
        std::sort(input_boxes.begin(), input_boxes.end(), [](BoundingBox a, BoundingBox b)
                  { return a.confidence > b.confidence; });
        std::vector<float> vArea(input_boxes.size());
        for (int i = 0; i < int(input_boxes.size()); ++i)
        {
            vArea[i] = (input_boxes.at(i).pts[2].x - input_boxes.at(i).pts[0].x + 1) * (input_boxes.at(i).pts[2].y - input_boxes.at(i).pts[0].y + 1);
        }
        for (int i = 0; i < int(input_boxes.size()); ++i)
        {
    ...
    private:
        ...
        void decode_infer(const float*& pred, std::vector<CenterPrior>& center_priors, float threshold, std::vector<std::vector<BoundingBox>>& results)
        {
    ...
    public:
        ...
        std::vector<BoundingBox> operator()(const cv::Mat &img)
        {
...

            {
                ...
                generate_grid_center_priors(this->input_size_[0], this->input_size_[1], this->strides_, center_priors);
