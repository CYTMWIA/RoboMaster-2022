...
constexpr float inv_sigmoid(float x) { return -std::log(1 / x - 1); }
...
void TRTModule::build_engine_from_onnx(const std::string &onnx_file)
{
  ...
  auto slice_layer =
      network->addSlice(*yolov5_output, Dims3{0, 0, 8}, Dims3{1, 15120, 1}, Dims3{1, 1, 1});
...
std::vector<BoundingBox> TRTModule::operator()(const cv::Mat &src) const
{
  ...
  // run model
  cudaMemcpyAsync(device_buffer[input_idx], x.data, input_sz * sizeof(float),
                  cudaMemcpyHostToDevice, stream);
  context->enqueue(1, device_buffer, stream, nullptr);
  cudaMemcpyAsync(output_buffer, device_buffer[output_idx], output_sz * sizeof(float),
                  cudaMemcpyDeviceToHost, stream);
  cudaStreamSynchronize(stream);

  // post-process [nms]
  std::vector<BoundingBox> rst;
