...
#undef ADD_CONFIDENCE
    ...
    void fix_boundingbox(BoundingBox &bbox, const cv::Mat &img)
    {
        ...
        if (pairs.size() && (contours.size() == 2 || pairs[0].confidence > 0.6))
        {
            ...
            bbox.pts[0] = cv::Point2f(range_col.start + b.pts[0].x/scale, range_row.start + b.pts[0].y/scale);
            bbox.pts[1] = cv::Point2f(range_col.start + b.pts[1].x/scale, range_row.start + b.pts[1].y/scale);
            bbox.pts[2] = cv::Point2f(range_col.start + b.pts[2].x/scale, range_row.start + b.pts[2].y/scale);
            bbox.pts[3] = cv::Point2f(range_col.start + b.pts[3].x/scale, range_row.start + b.pts[3].y/scale);
        }
    }
}
