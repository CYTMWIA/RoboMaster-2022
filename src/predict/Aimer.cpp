...
namespace rmcv::predict
{
    ...
    AimDeviation Aimer::aim_static(const Eigen::Matrix<double, 3, 1> &target_pos, float current_pitch)
    {
        ...
        // 计算过程还请看笔记《弹丸运动学》
        // 将xOy平面转到水平面
        float yzd = sqrt(ty * ty + tz * tz);
        float hx = tx, hy = yzd * cos(current_pitch), hz = yzd * sin(current_pitch);
        // 假设现在是竖直平面，平面上有【枪口】与【目标】两个点
        float x = sqrt(hx * hx + hy * hy); // 与目标在x轴（横轴）的距离
        float y = hz;                      // 与目标在y轴（纵轴）的距离

        float a = (-g_ * x * x) / (2 * bullet_speed_ * y), b = x / y;
        ...
        float pitch = theta - current_pitch;
